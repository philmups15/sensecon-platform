...
const cardHeadStyle = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid #D7E4E1' };
const rowStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 16px', borderBottom: '1px solid #E9F1EF', fontSize: 13 };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ ...cardStyle, overflow: 'hidden' }}>
        <div style={{ padding: '12px 16px', fontSize: 12.5, fontWeight: 700, borderBottom: '1px solid #D7E4E1' }}>Tenants</div>
        {tenants.map((t, i) => (
          <div key={i} style={{ ...rowStyle, borderBottom: i === tenants.length - 1 ? 'none' : rowStyle.borderBottom }}>
            <div style={{ fontWeight: 600 }}>{t.name}</div>
            <div style={{ color: '#52685F' }}>{t.plants} plants · {t.users} users</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <div style={{ ...cardStyle, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '12px 16px' }}>
            {userCount === null ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#78908A' }}><Spinner size={13} />Loading…</div>
            ) : (
            ...
            <div style={{ color: '#78908A', fontSize: 12.5, marginTop: 8 }}>Manage accounts, reset passwords, and control access from one place.</div>
          </div>
        </div>

        <div style={{ ...cardStyle, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', fontSize: 12.5, fontWeight: 700, borderBottom: '1px solid #D7E4E1' }}>Templates</div>
          {templates.map((tp, i) => (
        ...
        <div style={{ ...cardStyle, overflow: 'hidden' }}>
          ...
          {integrationsLoading && <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#78908A' }}><Spinner size={13} />Loading…</div>}
        ...
        <div style={{ ...cardStyle, overflow: 'hidden' }}>
          ...
          {auditLoading && <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#78908A' }}><Spinner size={13} />Loading…</div>}
          {!auditLoading && recentAudit.length === 0 && (
            <div style={{ padding: '12px 16px', fontSize: 13, color: '#78908A' }}>No activity recorded yet.</div>
          )}
          {recentAudit.map((a, i) => (
            <div key={a.entityId} style={{ padding: '10px 16px', borderBottom: i === recentAudit.length - 1 ? 'none' : '1px solid #E9F1EF', fontSize: 12.5 }}>
              <b>{a.who}</b> · {a.action}
              <div style={{ color: '#78908A', fontSize: 11, marginTop: 2 }}>{a.time}</div>
            </div>
          ))}
        </div>
      </div>
