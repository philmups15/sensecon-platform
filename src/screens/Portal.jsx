  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ background: '#FFFFFF', border: '1px solid #E4E8EB', borderRadius: 12, padding: 18 }}>
        {plants.length === 0 && <div style={{ fontSize: 13, color: '#9AA0A6' }}>No plants yet.</div>}
        {plants.map((p) => (
          <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0' }}>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{p.name}</div>
              <div style={{ fontSize: 12, color: '#6A7178' }}>{p.capacity} · PR {p.pr ? Math.round(p.pr * 100) + '%' : '—'}</div>
            </div>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E4E8EB', borderRadius: 12, overflow: 'hidden' }}>
        ...
        {history.length === 0 && <div style={{ padding: 16, fontSize: 13, color: '#9AA0A6' }}>No closed work orders yet.</div>}
        {history.map((w) => (
          <div key={w.entityId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 16px', borderBottom: '1px solid #F0F2F4', fontSize: 13 }}>
            <div>{w.title} <span style={{ color: '#9AA0A6', fontSize: 11 }}>{w.id}</span></div>
